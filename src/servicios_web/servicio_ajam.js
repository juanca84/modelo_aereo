...
const servicios = {
  obtenerArea: (codigo, correlativo, fechaSolicitud) => {
    const deferred = Q.defer();
    console.log(`codigo: ${codigo}`);
    console.log(`correlativo: ${correlativo}`);
    console.log(`fecha: ${fecha}`);
    //argumentos para consultar al servicio
    const url = `${ config.app.servicio_ajam.url }/${ codigo }`;
    const token = `Bearer ${ config.app.servicio_segip.token }`;
    const queryCorrelativo = `?correlativo=${ correlativo }`;
    const queryFechaSolicitud = `?fechaSolicitud=${ fechaSolicitud }`;
    // header
    const args = {
      headers: {
