...
configuracion.parches.forEach((parche) => {
  ...
  console.log("------------------------ Recorriendo parche");

  // Verifica la existencia del directorio destino.
  fs.exists(parche.ruta_destino, (resultadoLectura) => {
    console.log(`Archivo: verificando la existencia de la ruta destino >>>> ${parche.ruta_destino} >>>> ${resultadoLectura}`);
    ...
    if (resultadoLectura) {
      ...
      fs.readdirSync(rutaOrigen).filter((archivo) =>
        ...
        .forEach((archivo) => {
          ...
          fs.readFile(`${rutaOrigen}/${archivo}`, {
            flag: "r",
          }, (errorLeer, dataLeer) => {
            ...
            if (!errorLeer) {
              ...
              fs.writeFile(`${parche.ruta_destino}/${archivo}`, dataLeer, {
                flag: 'w',
              }, (errorEscritura) => {
